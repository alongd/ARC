...
groups. These enable molecules to be searched for common motifs (e.g., reaction sites).
"""
...
import arc.molecule.molecule as mol
from arc.exceptions import ActionError, ImplicitBenzeneError, UnexpectedChargeError
from arc.molecule.atomtype import ATOMTYPES, allElements, nonSpecifics, get_features, AtomType
from arc.molecule.graph import Vertex, Edge, Graph
...
class Group(Graph):
    ...
    def update_charge(self):
        ...
        for atom in self.atoms:
            if (len(atom.charge) == 1) and (len(atom.lone_pairs) == 1) and (len(atom.radical_electrons) == 1):
                ...
                # performed. If there are multiple charges are assigned, no update either.
    ...
    def merge(self, other):
        """
        Merge two groups to store them in a single :class:`Group` object. The merged :class:`Group` object is returned.
        """
    ...
    def get_element_count(self):
        ...
        """
        from arc.molecule.atomtype import allElements
    ...
    def standardize_atomtype(self):
        ...
        for atom in self.atoms:
            ...
            if filled_valency >= elements.PeriodicSystem.valence_electrons[self.symbol][element] - 1:
...
def merge_overlapping_benzene_rings(ring1, ring2, od):
    ...
    Returns: If rings are able to be merged, returns a new list of the merged ring, otherwise
    an empty list
    """
    new_ring = []
    # ring already complete
