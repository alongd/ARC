...
from arc.exceptions import ElementError
    ...
    def __init__(self, number, symbol, name, mass, isotope=-1, chemkin_name=None):
        ...
        if symbol in {'X','L','R','e'}:
            ...
            try:
                ...
                logger.error("RDkit doesn't know element {0} so covalent radius unknown".format(symbol))
                self.cov_radius = 0
...
class PeriodicSystem(object):
    ...
    valences = {'H+':0, 'e': 0, 'H': 1, 'He': 0, 'C': 4, 'N': 3, 'O': 2, 'F': 1, 'Ne': 0,
                'Si': 4, 'P': 3, 'S': 2, 'Cl': 1, 'Br': 1, 'Ar': 0, 'I': 1, 'X': 4, 'Li': 1, 'Na': 1, 'K': 1, 'Mg': 2, 'Ca': 2}
    valence_electrons = {'H+':0, 'e': 1, 'H': 1, 'He': 2, 'C': 4, 'N': 5, 'O': 6, 'F': 7, 'Ne': 8,
                         'Si': 4, 'P': 5, 'S': 6, 'Cl': 7, 'Br': 7, 'Ar': 8, 'I': 7, 'X': 4, 'Li': 1, 'Na': 1, 'K': 1, 'Mg': 2, 'Ca': 2}
    lone_pairs = {'H+':0, 'e': 0, 'H': 0, 'He': 1, 'C': 0, 'N': 1, 'O': 2, 'F': 3, 'Ne': 4,
                  'Si': 0, 'P': 1, 'S': 2, 'Cl': 3, 'Br': 3, 'Ar': 4, 'I': 3, 'X': 0, 'Li': 0, 'Na': 0, 'K': 0, 'Mg': 0, 'Ca': 0}
...
for key, value in bde_dict.items():
    q = value[0] * 1000  # Convert kJ/mol to J/mol
