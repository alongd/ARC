...
def get_reaction_family_products(rxn: 'ARCReaction',
    ...
    for family_label in family_labels:
        ...
        except (KeyError, ValueError, InvalidAdjacencyListError):
            logger.debug(f'Skipping family {family_label} due to unsupported group definition.')
...
def check_product_isomorphism(products: List['Molecule'],
    ...
    for i in range(len(products)):
        if not isomorphic[i]:
            ...
            if any(spc.mol.to_inchi() == inchi_a for spc in p_species):
                isomorphic[i] = True
