...
class ARCSpecies(object):
    ...
    `recent_md_conformer`   ``list``     A length three list containing the string format xyz of the recent conformer
                                           generated by MD, the energy in kJ/mol, and the number of MD runs.
                                           Used to detect when the MD algorithm converges on a single structure.
    ...
    `force_field`           ``str```     The force field to be used for conformer screening. The default is MMFF94.
                                           ...
                                           be generated, then force field parameters will be fitted for this molecule
                                           and conformers will be re-run with the fitted force field (recommended for
                                           drug-like species and species with many heteroatoms). Another option is
                                           specifying 'cheap', and the "old" RDKit embedding method will be used.
    ====================== ============= ===============================================================================
...
*   rotors_dict: {1: {'pivots': pivots_list,
    ...
    def number_of_heavy_atoms(self):
        ...
        if self._number_of_heavy_atoms is None:
            if self.mol is not None:
                ...
                self._number_of_heavy_atoms = len([line for line in self.get_xyz().splitlines() if line.split()[0] != 'H'])
            elif self.is_ts:
    ...
    def radius(self):
        ...
        if self._radius is None:
            coords, symbols, _, _, _ = get_xyz_matrix(self.get_xyz())
            # make sure the coordinates are centered at the center of mass:
    ...
    def generate_conformers(self, confs_to_dft=5):
        """
        Generate conformers
    ...
    def get_cheap_conformer(self):
        ...
        Returns:
            str, unicode: A reasonable conformer's xyz coordinates in string format.
        """
        num_confs = min(500, max(50, len(self.mol.atoms) * 3))
        rd_mol, rd_index_map = conformers.embed_rdkit(self.mol, num_confs=num_confs)
        xyzs, energies = conformers.rdkit_force_field(rd_mol=rd_mol, rd_index_map=rd_index_map, mol=self.mol,
                                                      force_field='MMFF94', return_xyz_strings=True)
    ...
    def get_xyz(self, get_cheap=True):
        ...
        Return:
             str, unicode: A string-format xyz coordinates.
        """
