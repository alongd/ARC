...
A job could (independently) consist a single task (e.g., optimizing a single well) or have multiple tasks (an array).
A single task job could run "incore" or be submitted to the queue.
A multitask job could be executed "incore" (taking no advantage of potential server parallelization)
or be submitted to the queue, in which case a powerful job array pipe will be automatically created.
"""
